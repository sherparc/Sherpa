...
| M3i ◐ | the manager sees what exists (first slice, §12): sherpa works with one repository — a nested repository anywhere in the tree (a harness clone under `.claude/`, a submodule, a vendored clone) stops `apply` and `adopt` with the way out named, the dry runs go on with a note, and `doctor` says it first with a `repositories` check and names the undecided home with a `layout` check (ADR-0045, supersedes 0037); `covered:` written by hand on an entry is a decision — kept across re-plans, honoured by `adopt` before its own linking, dropped with a note when the file is gone; only docs under `docs/modules/` link, one file per entry, ties cover nothing and are named in the gaps (ADR-0046); C1–C5 FAIL only in files sherpa generated, WARN `(yours)` elsewhere, `check --strict` for the old behaviour (ADR-0047). Open in M3i: F24 removal of own unchanged bytes (Q26), F25 orphan records in `adopt`, F26 content-only `harness_rev` (Q27), F32 dropped decisions named, F27/F28/F29/F30/F31 as fruit, F37 (Q33). | measured on the §12 repository: `apply` and `adopt` refuse (`.claude/` is a clone of its own) and name the way out, the dry run goes on; with the clone's files as this repository's harness (the first cut, measured before the refusal): four hand-set covers turn 39 proposed owner docs into 31, tied analysis notes are named instead of covering; `apply --yes` says `check: 0 FAIL, 42 WARN` and `sherpa check` exits 0 where 42 FAIL stood |
...
Gate: coverage ≥ 90 % for `src/sherpa/`, `pytest -q` green before every milestone. Status M3i (first slice): 407 tests, 98 %.
