...
| Corpus | real repos cloned next to this one (never named) | the e2e suite on `SHERPA_E2E_REPO` before every tag release (`/e2e-test`); smoke of every feature by hand: scan, plan, apply dry run, check; never in CI |
...
Gate: coverage ≥ 90 % for `src/sherpa/`, `pytest -q` and `pytest tests/e2e -q` green before every milestone. Status: 443 tests, 97 %, 31 end-to-end theses.
