...
Gate: coverage ≥ 90 % for `src/sherpa/`, `pytest -q` and `pytest tests/e2e -q` green before every milestone. Status: 453 tests, 98 %, 38 end-to-end theses. The test count in the README is read from the collector (`pytest --collect-only -q`) before it is written — thesis T22 checks it (§14 F61).
...
- Gate: 451 → 453 tests, 98 % (`TOTAL 3613 89`), 28 s; `ruff check` and `format --check` clean.
