...
Gate: coverage ≥ 90 % for `src/sherpa/`, `pytest -q` green before every milestone. Status M3i: 412 tests, 97 %.
...
12. ~~Milestone order (§7.3)?~~ Decided 2026-09-17: M2b → M3d → M5 → M6-lite → M4 → M6 → M3b → M7; revision 10
    inserted M3h and M7a before M5, revision 11 (§10 F6) put M5 back before M7a: M3h → M5 → M7a → …; revision 22 (ADR-0050) put the host
