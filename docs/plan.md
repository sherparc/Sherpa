...
Gate: coverage ≥ 90 % for `src/sherpa/`, `pytest -q` and `pytest tests/e2e -q` green before every milestone. Status: 460 tests, 98 %, 40 end-to-end theses. The test count in the README is read from the collector (`pytest --collect-only -q`) before it is written — thesis T22 checks it (§14 F61).
...
29. ~~**`status --exit-code` (§11 F31).**~~ Decided 2026-09-20 → ADR-0057, built as M3m: exit 2 when `apply` would write, on a stale plan or a torn state; a hand edit never counts; `apply --json` in the same contract. Recommendation: yes — exit 2 on drift or a stale plan, as `terraform plan
...
| F63 | **No machine-readable dry run.** | `status`, `check`, `doctor` have `--json`; `apply --dry-run` is prose (`render_actions`). M7a's `/sherpa-apply` would scrape it. | **Built as M3m** the same day → ADR-0057: `apply --json` and `status --exit-code` (Q29), Terraform's `plan -json` and `-detailed-exitcode`. |
...
M3l and M3m built the same day (revisions 37 and 38): the revision that means something, then the CI
contract (ADR-0057) — before M3h because the M7a plugin and every CI gate consume those two outputs and the
theses stop scraping prose. Next: the changelog (built 2026-09-21, ADR-0058), M2c's announcement, then M3h with Q28; Q37 after it as a
slice of its own.
