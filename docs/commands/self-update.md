...
Install the latest GitHub release with the installer that owns this copy — `uv tool`, `pipx` or `pip`. A clone
(editable install) is refused with the fix: `git pull`.
...
$ sherpa self-update --check
sherpa 0.6.0 is current (latest release: v0.6.0).
```
