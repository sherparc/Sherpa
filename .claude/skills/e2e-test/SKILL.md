...
| T04 | `scan` speed: files and seconds in the summary line; compare with the README's benchmark | README § scanner |
...
| T09 | a new checker FAIL rolls back completely: exit 1, `git status --short --ignored` empty, **no empty `.agents/`/`.claude/` left**, the next `apply` not blocked | ADR-0013/0032 |
...
| T19 | both homes present: dry runs assume `.agents` and say so; a write without a terminal refuses with the fix named | ADR-0036 |
...
python3 -c "import json;m=json.load(open('$R/.sherpa/codebase-model.json'));print(m['as_of'],m.get('trunk'))"
...
# T15 · T19 — without a state both homes exist and nothing is decided
REV=$(python3 -c "import json;print(json.load(open('$R/.sherpa/state.json'))['harness_rev'])")
rm "$R/.sherpa/state.json"
$S apply --dry-run "$R" 2>&1 | sed -n 1,2p                         # T19: assumes .agents and says so
$S apply --yes "$R" </dev/null 2>&1 | head -1; echo "exit=${PIPESTATUS[0]}"   # T19: refuses, names sherpa.toml [apply] home
